...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SWATT }

    address public immutable WATT;
    ...
    address public sWATT;
    uint public sWATTQueue; // Delays change to sWATT address
    ...
    constructor (
        ...
        require( _WATT != address(0) );
        WATT = _WATT;

        isReserveToken[ _DAI ] = true;
        reserveTokens.push( _DAI );

        // isLiquidityToken[ _WATTDAI ] = true;
        // liquidityTokens.push( _WATTDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for WATT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint WATT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( WATT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn WATT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IWATTERC20( WATT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sWATT ).balanceOf( msg.sender ); // Can only borrow against sWATT held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with WATT
        @param _amount uint
     */
    function repayDebtWithWATT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IWATTERC20( WATT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, WATT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( WATT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( WATT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns WATT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match WATT decimals
            value_ = _amount.mul( 10 ** IERC20( WATT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SWATT ) { // 9
            sWATTQueue = block.number.add( blocksNeededForQueue );
        } else return false;
